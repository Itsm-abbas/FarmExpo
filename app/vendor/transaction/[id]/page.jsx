...
export default function VendorTransactionPage() {
  ...
  const exportToPDF = async () => {
    ...
    try {
      // Dynamically import both jsPDF and autoTable
      ...
      doc.text("FARM EXPO", 105, 15, { align: "center" });
      ...
      doc.text(`Vendor: ${data.vendor.name}`, 14, 50);
      doc.text(
      ...
      doc.text(`Report Date: ${new Date().toLocaleDateString()}`, 14, 62);
      doc.text(`Total Transactions: ${data.transactions.length}`, 14, 68);
      ...
      doc.text(
        `Total Debits: ${data.vendor.currency} ${totalDebits.toLocaleString()}`,
        120,
        56
      );
      doc.text(
        `Net Balance: ${data.vendor.currency} ${(
          totalCredits - totalDebits
        ).toLocaleString()}`,
        120,
        62
      ...
      Swal.fire({
        ...
        icon: "success",
        title: "PDF Exported!",
        text: "Beautiful transaction ledger has been downloaded",
        showConfirmButton: false,
        timer: 2000,
        background: "rgb(var(--color-background))",
        color: "rgb(var(--color-text))",
